...
Function MakeValidBMName(strIn As String)
    ...
    For i = 1 To Len(strIn)
        charcode = AscW(Mid$(strIn, i, 1)) 	' 使用AscW函数获取字符的Unicode码点

        If charcode < 0 Then
            charcode = 65536 + charcode
        End If

        If (charcode >= 49 And charcode <= 57) Or _
                (charcode >= 65 And charcode <= 90) Or _
                (charcode >= 97 And charcode <= 122) Then	'数字字母

            tempStr = tempStr & Mid $(strIn, i, 1)
        ElseIf charcode >= 19968 And charcode <= 40959 Then	'汉字
            tempStr = tempStr & Mid $(strIn, i, 1)
        Else
            tempStr = tempStr & "_"
        End If
